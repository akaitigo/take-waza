...
function isWeavingNode(value: unknown): value is WeavingNode {
  ...
  return (
    ...
    typeof v["y"] === "number" &&
    typeof v["z"] === "number" &&
    (v["crossing"] === "over" || v["crossing"] === "under")
  );
}
...
function isWeavingEdge(value: unknown): value is WeavingEdge {
  ...
  return (
    typeof v["id"] === "number" &&
    typeof v["from_node"] === "number" &&
    typeof v["to_node"] === "number" &&
...
export function isWeavingGraph(value: unknown): value is WeavingGraph {
  ...
  return (
    ...
    v["nodes"].every(isWeavingNode) &&
    Array.isArray(v["edges"]) &&
    v["edges"].every(isWeavingEdge) &&
    typeof v["bamboo_width"] === "number" &&
    typeof v["bamboo_thickness"] === "number" &&
...
async function loadWasmModule(): Promise<WasmModule | null> {
  try {
    ...
    const mod =
      (await import("../../wasm-engine/pkg/wasm_engine.js")) as unknown as WasmModule;
    await mod.default();
    return mod;
  } catch {
    return null;
...
export function wasmGeneratePattern(
  ...
  try {
    const json = wasmState.module.generate_pattern_json(
      patternName,
      width,
      thickness,
      count,
 ...
 */
export function wasmTransformPattern(
  graph: WeavingGraph,
  newWidth: number,
  newThickness: number,
): WeavingGraph | null {
  ...
  try {
    ...
    const json = wasmState.module.transform_pattern_json(
      graphJson,
      newWidth,
      newThickness,
    );
 ...
 * WASM が利用不可の場合は null を返す。
 */
export function wasmDecomposeSteps(
  graph: WeavingGraph,
): WasmStepSequence | null {
  if (wasmState.status !== "ready") return null;
  try {
    const graphJson = JSON.stringify(graph);
